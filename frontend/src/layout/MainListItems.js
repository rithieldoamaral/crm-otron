...
const MainListItems = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Guard: durante logout, user.companyId pode virar undefined brevemente
      // e o request /companies/listPlan/undefined dispara 401/400.
      if (!companyId) return;
      try {
        ...
        // 401 esperado durante logout (token invalidado + request in-flight).
        // O interceptor em useAuth já cuida da limpeza de estado + redirect.
        if (err?.response?.status !== 401) {
          console.error("Erro ao carregar plan configs:", err);
        }
      }
    }
    fetchData();
  ...
  return (
    <div className={classes.menuWrapper} onClick={drawerClose}>
      ...
      {/* ── DIA A DIA — visível para todos os perfis ───────────── */}
      <Can
        role={user.profile}
        ...
        no={() => (
          <>
            <ListSubheader hidden={collapsed} className={classes.subheader} inset color="inherit">
              {i18n.t("mainDrawer.sections.diaDia")}
            </ListSubheader>
      ...
      {/* ── GESTÃO + CONFIGURAÇÕES + AVANÇADO + SISTEMA — admins ──
       *
       * NOTA: A seção "DIA A DIA" é renderizada acima pelo primeiro <Can> com
       * handler `no={...}`, que dispara para qualquer perfil que não tenha a
       * permission `drawer-service-items:view`. Como atualmente NENHUM perfil
       * declara essa permission em rules.js, todo mundo (inclusive admin) cai
       * no `no()` e vê DIA A DIA. Por isso este bloco NÃO repete DIA A DIA —
       * evita duplicação. Se um dia for desejado restringir DIA A DIA por
       * perfil, basta adicionar a permission ao perfil correspondente.
       */}
      <Can
        role={user.profile}
        ...
        yes={() => (
          <>
            {/* GESTÃO */}
            <ListSubheader hidden={collapsed} className={classes.subheader} inset color="inherit">
              {i18n.t("mainDrawer.sections.gestao")}
            </ListSubheader>
            ...
            <ListItemLink
              to="/finance"
              primary={i18n.t("mainDrawer.listItems.financeAnalytics")}
            ...
            <ListItemLink
              to="/retencao"
              primary={i18n.t("mainDrawer.listItems.retention")}
              icon={<FiRefreshCw size={18} />}
              isActive={isActivePath('/retencao')}
              collapsed={collapsed}
            />
            <ListItemLink
              to="/tags"
            ...
            {/* CONFIGURAÇÕES */}
            <ListSubheader hidden={collapsed} className={classes.subheader} inset color="inherit">
              {i18n.t("mainDrawer.sections.configuracoes")}
            </ListSubheader>
            ...
            <ListItemLink
              to="/services"
              primary={i18n.t("mainDrawer.listItems.serviceCatalog")}
              icon={<FiGrid size={18} />}
            ...
            <ListItemLink
              ...
              isActive={isActivePath('/packages')}
              collapsed={collapsed}
            />
            <ListItemLink
              to="/files"
            ...
            <ListItemLink
              to="/financeiro"
              primary={i18n.t("mainDrawer.listItems.mensalidadeCrm")}
              ...
              <>
                <ListSubheader hidden={collapsed} className={classes.subheader} inset color="inherit">
                  <ListItem
                    button
                    dense
                    onClick={(e) => { e.stopPropagation(); setOpenCampaignsSubmenu((v) => !v); }}
                    style={{ padding: 0, borderRadius: 6 }}
                  >
                    <ListItemText
                      primary="Campanhas"
                      primaryTypographyProps={{ style: { fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "inherit" } }}
                    />
                    {openCampaignsSubmenu ? <FiChevronUp size={14} /> : <FiChevronDown size={14} />}
                  </ListItem>
                </ListSubheader>
                <Collapse in={openCampaignsSubmenu} timeout="auto" unmountOnExit>
                  <List component="div" disablePadding>
                    <ListItemLink
                      to="/contact-lists"
                      primary={`1. ${i18n.t("mainDrawer.listItems.contactLists")}`}
                      icon={<FiUsers size={18} />}
                    ...
                    <ListItemLink
                      to="/campaigns-config"
                      primary={`2. ${i18n.t("mainDrawer.listItems.campaignsConfig")}`}
                      icon={<FiSettings size={18} />}
                    ...
                    <ListItemLink
                      to="/campaigns"
                      primary={`3. ${i18n.t("mainDrawer.listItems.campaigns")}`}
                      icon={<FiMail size={18} />}
            ...
            <ListSubheader hidden={collapsed} className={classes.subheader} inset color="inherit">
              <ListItem
                button
                dense
                onClick={(e) => { e.stopPropagation(); setOpenCampaignSubmenu((v) => !v); }}
                style={{ padding: 0, borderRadius: 6 }}
              >
                <ListItemText
                  primary={i18n.t("mainDrawer.sections.avancado")}
                  primaryTypographyProps={{ style: { fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "inherit" } }}
                />
                {openCampaignSubmenu ? <FiChevronUp size={14} /> : <FiChevronDown size={14} />}
              </ListItem>
            </ListSubheader>
