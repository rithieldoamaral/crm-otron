...
import { Menu, MenuItem, Dialog, DialogTitle, DialogContent, DialogActions, CircularProgress, Typography } from "@material-ui/core";
...
const Contacts = () => {
  ...
  const [summaryContact, setSummaryContact] = useState(null);
  ...
  const handleSummarize = async (contact) => {
    ...
    try {
      const { data } = await api.post(`/contacts/${contact.id}/summarize`);
      setSummaryData(data);
    } catch (err) {
      toastError(err);
    ...
    <MainContainer className={classes.mainContainer}>
        ...
        <Table size="small">
          ...
          <TableBody>
            <>
                  ...
                  <TableCell align="center">
                    <Tooltip title="Resumir cliente (IA)">
                      <IconButton
                        size="small"
                        onClick={() => handleSummarize(contact)}
                        color="primary"
                      >
                        <SubjectIcon />
                      </IconButton>
      ...
      <Dialog open={summaryOpen} onClose={() => setSummaryOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          ...
          ) : summaryData?.clienteEncontrado && summaryData?.resumo ? (
            <Typography variant="body2" component="pre" style={{ whiteSpace: "pre-wrap", fontFamily: "inherit" }}>
              {summaryData.resumo}
            </Typography>
          ) : (
            <Typography color="textSecondary" variant="body2">
              {summaryData?.erro || "Não foi possível gerar o resumo."}
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSummaryOpen(false)} color="primary">
            Fechar
          </Button>
        </DialogActions>
      </Dialog>
