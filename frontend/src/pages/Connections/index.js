...
const useStyles = makeStyles(theme => ({
  ...
  secretaryChip: {
    backgroundColor: theme.palette.type === 'dark' ? "#1a3a4a" : "#e0f4ff",
    color: theme.palette.type === 'dark' ? "#7ecfee" : "#0077aa",
    fontSize: 11,
    height: 22,
  ...
  secretaryToggle: {
    marginTop: theme.spacing(1),
    marginLeft: 0,
    "& .MuiFormControlLabel-label": { fontSize: "0.78rem", color: theme.palette.text.secondary },
  },
}));
...
const Connections = () => {
  ...
  const handleToggleSecretary = async (whatsApp) => {
    try {
      await api.put(`/whatsapp/${whatsApp.id}`, {
        isSecretaryChannel: !whatsApp.isSecretaryChannel,
      });
      toast.success(
        !whatsApp.isSecretaryChannel
          ? "Canal marcado como Secretária IA"
          : "Canal removido da Secretária IA"
      );
    } catch (err) {
      toastError(err);
    }
                      ...
                      <CardContent className={classes.cardContent}>
                        ...
                        <Can
                          ...
                          yes={() => (
                            <Box>
                              <FormControlLabel
                                className={classes.secretaryToggle}
                                control={
                                  <Switch
                                    size="small"
                                    checked={!!whatsApp.isSecretaryChannel}
                                    onChange={() => handleToggleSecretary(whatsApp)}
                                    color="primary"
                                  />
                                }
                                label="Canal da Secretária IA"
                              />
                              {whatsApp.isSecretaryChannel && (
                                <Chip
                                  ...
                                  label="Secretária IA"
                                  size="small"
                                  className={classes.secretaryChip}
                                />
                              )}
                            </Box>
                          )}
