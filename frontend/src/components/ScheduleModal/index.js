...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	const initialState = {
		...
		// Multi-profissional (módulo Google Calendar)
		professionalId: "",
		serviceId: "",
	};
	...
	// Multi-profissional: opções para os dropdowns do modal
	...
	useEffect(() => {
		...
		// Carrega profissionais e serviços para dropdowns do modal.
		// Endpoints do módulo Google Calendar — campos ficam vazios se empresa
		// ainda não configurou o módulo (404/403 esperado nesse caso).
		const fetchProfessionalsAndServices = async () => {
			try {
				...
				setScheduleServices(Array.isArray(servRes.data) ? servRes.data : []);
			} catch (err) {
				// Log ajuda debug sem poluir UI — o usuário entende "campos não aparecem"
				// mas developer pode investigar se os endpoints deveriam estar respondendo.
				console.warn("[ScheduleModal] Não foi possível carregar profissionais/serviços:", err?.message || err);
			}
		};
				...
				(async () => {
					...
					setSchedule(prevState => {
						return {
							...
							// Formik precisa de string vazia (não null) para controlled Select funcionar
					...
					{({ touched, errors, isSubmitting, values, setFieldValue }) => (
						<Form>
							<DialogContent dividers>
								...
								{/*
								  Multi-profissional: dropdowns Profissional + Serviço.
								  Só aparecem se empresa configurou o módulo Google Calendar
								  (tem pelo menos 1 profissional conectado OU 1 serviço cadastrado).
								  Why: em empresas que não usam, poluir o modal é pior que ausência.
								*/}
								{(professionals.length > 0 || scheduleServices.length > 0) && (
									<>
										<div className={classes.multFieldLine}>
											<FormControl variant="outlined" margin="dense" fullWidth>
												<InputLabel id="professionalId-selection-label">Profissional</InputLabel>
												<Field
													as={Select}
													label="Profissional"
													labelId="professionalId-selection-label"
													id="professionalId"
													name="professionalId"
												>
													<MenuItem value=""><em>Nenhum</em></MenuItem>
													{professionals.map((p) => (
														<MenuItem key={p.userId} value={p.userId}>{p.name}</MenuItem>
													))}
												</Field>
											</FormControl>
											<FormControl variant="outlined" margin="dense" fullWidth>
												<InputLabel id="serviceId-selection-label">Serviço</InputLabel>
												<Field
													as={Select}
													label="Serviço"
													labelId="serviceId-selection-label"
													id="serviceId"
													name="serviceId"
												>
													<MenuItem value=""><em>Nenhum</em></MenuItem>
