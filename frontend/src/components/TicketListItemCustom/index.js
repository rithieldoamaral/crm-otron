...
const TicketListItemCustom = ({ ticket }) => {
  ...
  // Keyboard shortcut T → open transfer modal (only when this ticket is selected)
  const isSelected = ticketId && +ticketId === ticket.id;
  useEffect(() => {
    if (!isSelected || ticket.status !== "open") return;
    const onKeyDown = (e) => {
      if (
        e.key === "t" &&
  ...
  }, [isSelected, ticket.status]);

  const getWaitAlertMinutes = () => {
    if (ticket.status !== "open" || !ticket.updatedAt) return null;
    const elapsed = Math.floor((Date.now() - new Date(ticket.updatedAt).getTime()) / 60000);
  ...
  const waitAlert = getWaitAlertMinutes();
  ...
  return (
    <>
      ...
      >
        {(() => {
          // Quando o canal é do agente IA e o ticket ainda está sob controle do bot
          // (chatbot=true), o badge prioriza identificar o agente em vez da fila.
          // Após transferência para humano, chatbot vira false e o badge volta a ser da fila.
          const isAgentControlled =
            ticket.whatsapp?.isAgentChannel && ticket.chatbot;
          const badgeLabel = isAgentControlled
            ? "AGENTE IA"
            : ticket.queue?.name || "SEM FILA";
          const badgeColor = isAgentControlled
            ? "#7B1FA2"
            : ticket.queue?.color || grey[500];
          return (
            <Tooltip arrow placement="left" title={badgeLabel}>
              <span
                style={{ backgroundColor: badgeColor }}
                className={classes.ticketQueueColor}
              >
                <span className={classes.queueLabel}>{badgeLabel}</span>
              </span>
            </Tooltip>
          );
        })()}
          ...
          secondary={
            <>
              ...
              <Box display="flex" flexWrap="wrap" gap="4px" mt={0.5} alignItems="center">
                ...
                {waitAlert && (
                  <Tooltip title={`Aguardando há ${waitAlert} sem resposta`}>
                    <span className={classes.waitAlertBadge}>
                      <AlarmIcon />
                      {waitAlert}
                    </span>
                  </Tooltip>
