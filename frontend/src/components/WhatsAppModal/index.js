...
const WhatsAppModal = ({ open, onClose, whatsAppId }) => {
  ...
  const handleSaveWhatsApp = async (values) => {
    const whatsappData = {
      ...
      promptId: null, // Campo legacy Open.AI — desativado, sempre null
    };
      ...
      >
        ...
        >
          {({ values, touched, errors, isSubmitting }) => {
            // Canais de Agente IA ou Secretária não usam mensagens de fluxo manual
            const isSpecialChannel = values.isAgentChannel || values.isSecretaryChannel;
            ...
            <Form>
              <DialogContent dividers>
                <div className={classes.multFieldLine}>
                  <Grid spacing={2} container>
                    ...
                    <Grid style={{ paddingTop: 15 }} item>
                      <FormControlLabel
                        control={
                          <Field name="isAgentChannel">
                            {({ field, form }) => (
                              <Switch
                                color="secondary"
                                checked={!!field.value}
                                onChange={(e) => form.setFieldValue("isAgentChannel", e.target.checked)}
                              />
                            )}
                          </Field>
                        }
                        label="Canal do Agente IA"
                      />
                    </Grid>
                    <Grid style={{ paddingTop: 15 }} item>
                      <FormControlLabel
                        control={
                          <Field name="isSecretaryChannel">
                            {({ field, form }) => (
                              <Switch
                                color="secondary"
                                checked={!!field.value}
                                onChange={(e) => form.setFieldValue("isSecretaryChannel", e.target.checked)}
                              />
                            )}
                          </Field>
                        }
                        label="Canal Secretária"
                      />
                    </Grid>
                  </Grid>
                </div>

                {/* Mensagens de fluxo manual — ocultadas para canais de IA */}
                {!isSpecialChannel && (
                  <>
                    <div>
                      <Field
                ...
                {/* Redirecionamento de fila — oculto para canais de IA */}
                {!isSpecialChannel && (
                  <div>
                ...
                {/* Encerrar chats abertos — visível para todos os canais */}
                <Grid spacing={2} container>
