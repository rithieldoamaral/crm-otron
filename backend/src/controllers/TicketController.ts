...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  // A aba "Secretária" (status="secretary") é privilégio exclusivo do admin da
  // empresa: expõe a conversa do admin com a Secretária IA (cancelar/fechar
  // tickets, dados financeiros). O frontend esconde a aba e o realtime (socket)
  // já é admin-only, mas este endpoint REST era craftável por um não-admin da
  // MESMA empresa. Gate no servidor para fechar o vazamento no fetch inicial.
  if (status === "secretary" && req.user.profile !== "admin") {
