...
const SimpleListService = async ({ name, companyId }: SearchContactParams): Promise<Contact[]> => {
  let options: FindOptions = {
    // Seleciona apenas as colunas necessárias para o autocomplete.
    // Isso evita falhas quando colunas novas (ex: birthday, marketingOptOut)
    // ainda não existem no banco (migrations pendentes).
